...
def test_duckdb_projection_matches_aliasing_expectation(tmp_path):
    ...
    con.execute(
        ...
        '("A b" integer, "c-d" integer, "_PUBLICATION_DATE" varchar)'
    )
    con.execute("insert into raw_data values (1, 2, '20260519T103600')")

    select_sql = render_alias_select_sql(
        ["A b", "c-d", "_PUBLICATION_DATE"], macro_duckdb_path
    )
...
def test_acronyms_remain_intact_in_aliasing(tmp_path):
    ...
    con.execute(
        "create table raw_data "
        '("NHSNumber" integer, "ODSCode" integer, "XMLParserVersion" integer)'
    )
    con.execute("insert into raw_data values (1, 2, 3)")

    select_sql = render_alias_select_sql(
        ["NHSNumber", "ODSCode", "XMLParserVersion"], macro_duckdb_path
    )
