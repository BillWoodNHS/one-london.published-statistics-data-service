...
from .download_and_normalize import build_artifact, normalize_payload_to_csv, normalize_to_csv
...
def _write_audit_record(artifact: LoadArtifact) -> None:
    ...
    payload = json.dumps(
        {
            "_INGESTED_AT": __import__("datetime").datetime.utcnow().isoformat(timespec="seconds") + "Z",
...
def execute_ingestion() -> Dict[str, List[str]]:
    ...
    for config in configs:
        effective_manual_prefix = config.fallback.manual_drop_path.strip("/") if config.fallback.manual_drop_path else manual_prefix
        ...
        for item in discovered:
            ...
            artifact = build_artifact(item, filename, content_hash, acquisition_method="automated")
        ...
        for target in config.targets:
            ...
            manual_candidates = discover_manual_files(config, target, effective_manual_prefix)
            for candidate in manual_candidates:
                payload = download_blob_bytes(candidate.source_url)
                filename, csv_payload, content_hash = normalize_payload_to_csv(candidate.link_text, payload)
